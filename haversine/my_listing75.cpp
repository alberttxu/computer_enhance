...
int main(int argc, char** argv)
{
   ...
   assert(jsonfile);
   u64 cputime_start = ReadCPUTimer();
   double avg_json = parse_jsonfile(jsonfile);
   u64 cputime_parsedone = ReadCPUTimer();
   printf("json parse time: %llu ticks, %f s\n",
         elapsedticks(cputime_start, cputime_parsedone),
         elapsedtime(cputime_start, cputime_parsedone));
   fclose(jsonfile);
   ...
   if (argc == 3)
   {
      ...
      assert(answersfile);
      u64 cputime_answersstart = ReadCPUTimer();
      double avg_answer = parse_answersfile(answersfile);
      u64 cputime_answersdone = ReadCPUTimer();
      fclose(answersfile);

      printf("error = %f\n", avg_answer - avg_json);
      printf("answers parse time: %llu ticks, %f s\n",
            elapsedticks(cputime_answersstart, cputime_answersdone),
            elapsedtime(cputime_answersstart, cputime_answersdone));
   }
